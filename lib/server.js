  ...
  this.init(function () {
    ...
    _this.server = net.createServer(function (connection) {
      //Now we are listening!
			...
			let existingDevice = _this.devices.find(device => device.connection === connection)
			if (existingDevice) {
				// Si ya existe un dispositivo, asignarlo a la conexión
        console.log('Existing device')
				connection.device = existingDevice;
			} else {
				// Si no existe, crear uno nuevo
        console.log('New device')
				connection.device = new Device(_this.getAdapter(), connection, _this);
				_this.devices.push(connection.device); // Asegurar que se agregue el nuevo dispositivo a la lista
			}
